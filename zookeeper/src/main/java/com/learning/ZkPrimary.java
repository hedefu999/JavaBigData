...
public class ZkPrimary {
     ...
     *
     */
    static class ZooKeeperPrinciples{
        //运行此程序得到 NoAuthException
        static void authFailed1() throws Exception{
            ...
            ZooKeeper zkClient = new ZooKeeper(connectString, 3000, watcher);
            zkClient.addAuthInfo("digest", "taokeeper:true".getBytes());
            zkClient.create("/zk-book", "".getBytes(), ZooDefs.Ids.CREATOR_ALL_ACL, CreateMode.EPHEMERAL);

            ZooKeeper zkClient_error = new ZooKeeper(connectString, 3000, watcher);
            zkClient_error.addAuthInfo("digest", "taokeeper:error".getBytes());
            zkClient_error.getData("/zk-book", true, null);
        }

        //addAuthInfo是使用错误的schema，将得到AuthFailException 同时Watcher会收到AuthFailed事件，上面的方法没有AuthFailed事件发出
        static void authFailed2() throws Exception{
            Watcher watcher = new Watcher() {
                @Override
                public void process(WatchedEvent event) {
                    logger.info("state = {}, type = {}", event.getState(), event.getType());
    ...
    static void AboutDigestAuthenticationProvider() throws Exception{
        String encrypt = DigestAuthenticationProvider.generateDigest("hedefu:hedefu999");
        System.out.println(encrypt);
    }

    /*
    演示下Jute的用法
    - 序列化实体类需要实现 Record 接口的 serialize 和 deserialize 方法
    - 构建序列化器 BinaryOutputArchive
    - 调用实体类的serialize方法，将对象序列化到指定tag中： MockReqHeader 对象序列化到 header
    - 调用实体类的deserialize方法，从指定tag中反序列化出数据内容
     */
    @Data    @NoArgsConstructor    @AllArgsConstructor
    static class MockReqHeader implements Record {
    ...
    /*
    Jute的 OutputArchive/InputArchive分别对应Jute底层的序列化器和反序列化器接口，
    目前Jute有 BinaryOutputArchive、CsvOutputArchive/XmlOutputArchive 三种实现
    ZK源码中的 File generated by hadoop record compiler. Do not edit 注释，表示Jute使用代码生成器为这些类定义生成的实际编程语言的类文件
    Jute使用 JavaGenerator 来生成相应的Java类文件,生成的Java类都会实现Record接口
     */
    static void juteSerializeHowto() throws Exception{
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        BinaryOutputArchive boa = BinaryOutputArchive.getArchive(baos);
        new MockReqHeader(0x4353454, "ping").serialize(boa, "header");
        ByteBuffer bb = ByteBuffer.wrap(baos.toByteArray());
        ByteBufferInputStream bbis = new ByteBufferInputStream(bb);
        BinaryInputArchive archive = BinaryInputArchive.getArchive(bbis);
        MockReqHeader header2 = new MockReqHeader();
        header2.deserialize(archive, "header");
        bbis.close();
        baos.close();
